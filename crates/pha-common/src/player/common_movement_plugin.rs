...
use pha_protocol::{
    component::Player, components::look_orientation::LookOrientation, input::NetworkedInput,
};
...
fn move_player(
    ...
    q_player_look: Query<(&Parent, &LookOrientation)>,
    time: Res<Time>,
    ...
    for (player_entity, action_state, mut velocity) in q_player.iter_mut() {
        if let Some(movement) = action_state.dual_axis_data(&NetworkedInput::Move) {
            ...
            for (parent, look_orientation) in &q_player_look {
